...
plot_function <- function (minx, maxx, filename, directory, stepfind, plot_dir,i,qsteptime) {
...
setwd(plot_dir)
#tiff(paste(i, ".tiff",sep=""), width = 15, height = 10, units = 'in', res = 300,compression="lzw")
plot(time3, yfilt3, col = "black", xlab="Time, s", ylab="Displacement, nm",
type="p", xlim=c(minx,maxx), main=dir, ylim=c(miny,maxy), yaxt="n",pch=19,cex=0.5)
#lines(time3,xfilt3,col = "blue")
at <- seq(from = miny, to = maxy, by = int)
axis(side = 2, at = at)
abline(h=seq(from=miny, to=maxy,by=int), NULL, col="gray", lty=3)
#lines(rep(qsteptime,200),1:200,col="red",lty=2)
...
yy1 <- bthr3$mean1
yy2 <- bthr3$mean2
mean1 <- rep( bthr3$mean1,length(time3) )
mean2 <- rep( bthr3$mean2,length(time3) )
time <- rep( bthr3$time,length(time3) )

return(data.frame(time3,yfilt3,time,mean1,mean2))
