...
#2018 04 23
#modified to run when called from backslips_plot_fit_line.R
#2018 04 23 fits a line to chosen part of the trace 
#(click on the beginning and the end, then right click "stop")

plot6_function <- function (minx, maxx, filename, directory, stepfind,qstep) {

#minx <- qstep$time-0.01
#maxx <- qstep$time+0.01
#directory <- file_dir2
#stepfind <- 1
...
###added to ge rid of data filtering
yfilt <- y
xfilt <- x
###
...
p <- yfilt3 < qstep$mean2+(qstep$amplitude*cc) & yfilt3 > qstep$mean1-(qstep$amplitude*cc) &
time3 > qstep$time - tt & time3 < qstep$time + tt

#p <- time3 >= coord_x[1] & time3 <= coord_x[2]

xtofit <- time3[p]
ytofit <- yfilt3[p]

model <- lm(ytofit~xtofit)

dffit <- data.frame(x=xtofit)
dffit$y <- predict(model,dffit)
lines(dffit$x, dffit$y,col="red",lwd="l",type="l")
#lines(xtofit,ytofit,type="p",col=4)
...
lines(bthr3$time,bthr3$tscore, col="red", lwd="2")
curve(x*thr/x, add=T)
